...
//리파지토리로 CRUD 작업 (Create, Read, Update, Delete)
@SpringBootTest
...
//		Memo memo = new Memo(0,"새글입니다");
	...
	@Test
	public void 데이터단건조회() {
		//결과값을 반환받을 때는 함수의 선언부를 확인할것
		//그리고 그타입을 대입연산자 앞부분에 변수로 선언
		//findById: 단건 데이터를 조회하는 함수
		Optional<Memo> optional = memoRepository.findById(1); //해당 엔티티(Table)의 key(pk 타입)값
